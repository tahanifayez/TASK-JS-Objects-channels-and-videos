...
function channelHasVideo(videoTitle, channel) {
  const flimname = channel.videos.some((video) => video.title == videoTitle);
  return flimname;
}
...
function getChannelByName(channelName, channels) {
  const Naming = channels.find((c) => c.name == channelName);
  return Naming;
}
...
function getChannelByVideoTitle(videoTitle, channels) {
  // const titling = channels.find((t) => t.videoTitle == videoTitle);
  // return titling;

  let x = channels.find((channel) =>
...
function searchChannels(query, channels) {
  ...
  // channels.fliter((q) => q.channel);

  let y = channels.filter(
    (channel) =>
